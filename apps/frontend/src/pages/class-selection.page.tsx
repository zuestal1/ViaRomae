...
import { Coins, Compass, HeartHandshake, Shield } from "lucide-react";
import type { ClassSelectionState, SelectablePlayerClass } from "@jlw/contracts";
import { api } from "../lib/api.js";

const CLASS_INFO = {
  GARDIST: { name: "Gardist", role: "Schützt das Team", hp: 120, attack: 8, defense: 12, Icon: Shield },
  MÖNCH: { name: "Mönch", role: "Heilt und unterstützt", hp: 100, attack: 7, defense: 9, Icon: HeartHandshake },
  HÄNDLER: { name: "Händler", role: "Handel und Versorgung", hp: 100, attack: 8, defense: 8, Icon: Coins },
  SPÄHER: { name: "Späher", role: "Erkundung und Fernkampf", hp: 90, attack: 12, defense: 7, Icon: Compass },
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2">
          {(Object.keys(CLASS_INFO) as SelectablePlayerClass[]).map((key) => {
            ...
            return <button key={key} disabled={!available || mutation.isPending} onClick={() => setSelected(key)} className={`rounded-xl border p-5 text-left transition ${active ? "border-[#e8943f] bg-[#cd7f32]/15" : "border-[#cd7f32]/25 bg-[#0d0d1a]"} disabled:cursor-not-allowed disabled:opacity-40`}>
              ...
              <div className="mt-4 flex gap-4 text-xs"><span>LP <b>{info.hp}</b></span><span>ANG <b>{info.attack}</b></span><span>VER <b>{info.defense}</b></span></div>
            </button>;
          })}
        </div>
